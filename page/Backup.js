...
const Backup = ({navigation}) => {
    ...
    useEffect(() => {
        ...
        startup().then().catch(() => ToastAndroid.show('發生錯誤請, 檢查網絡狀態', ToastAndroid.SHORT));

        DB.transaction(function(tr){
            tr.executeSql('SELECT value FROM Setting WHERE Target IN(\'AutoBackup\', \'AutoBackup_cycle\')', [], (tr, rs) => {
                    ...
                    <View>
                        ...
                        {isLogin ?
                            <Animated.View style={[style.backup, {backgroundColor: BG_color}]} entering={FoldIn} exiting={FoldOut}>
                                <View style={style.logo}>
                                    <Title>自動備份:</Title>
                                    <Switch value={autoBackupEnable} color={Color.primaryColor} onValueChange={switchAutoBackup}/>
                                </View>
                                <RadioGroup containerStyle={{
                                    ...
                                    <RadioButton value={'Day'} label={'每日'} color={Color.primaryColor} selected={autoBackupCycle === 'Day'}/>
                                    <RadioButton value={'Week'} label={'每週'} color={Color.primaryColor} selected={autoBackupCycle === 'Week'}/>
                                    <RadioButton value={'Month'} label={'每月'} color={Color.primaryColor} selected={autoBackupCycle === 'Month'}/>
                                </RadioGroup>
                            </Animated.View> : null}
                    </View>
...
const doBackup = async(folderID) => {
    ...
    await RNFS.copyFile(CachesDirectoryPath + '/../databases/' + dbname[0] + '.db', CachesDirectoryPath + '/backup.tmp.db'); //copy to Caches folder
    ...
    await AsyncStorage.setItem('Last_Backup', new Date('1995-12-17T03:24:00').toISOString());
    ...
    DB.transaction(function(tr){
        tr.executeSql('SELECT value FROM Setting WHERE Target IN(\'AutoBackup\', \'AutoBackup_cycle\')', [], async(tr, rs) => {
            ...
            if(Last_Backup && setting.enable){
                ...
                if((setting.cycle === 'Day' && diff >= (1000 * 60 * 60 * 24))
