...
Upgrade React Navigation to the next major version and handle all required breaking changes while preserving existing
navigation behavior.
...
If, after reading the relevant navigator and the upgrade reference, you cannot determine with high confidence whether an
API changed or how to update it, pause and ask the user before editing code.
...
- For any other version, follow the [upgrade guides](https://reactnavigation.org/docs/upgrade-guides.md) to move to 7.x
  or 8.x first.
